...
public class PostalCode {
     ...
     * 
     */
    public static String isValidPostalCode(String postalCode) {
        if (postalCode == null) {
            throw new NullPointerException("No postal code was given!");
        } else if (Integer.valueOf(postalCode.trim().substring(0, 4)) > 999 && Integer.valueOf(postalCode.trim().substring(0, 4)) <= 9999 && 
        postalCode.trim().substring(4).trim().length() == 2 && 'A' <= postalCode.trim().substring(4).trim().toUpperCase().charAt(0) &&
        postalCode.trim().substring(4).trim().toUpperCase().charAt(0) <= 'Z' &&
        'A' <= postalCode.trim().substring(4).trim().toUpperCase().charAt(0) && postalCode.trim().substring(4).trim().toUpperCase().charAt(0) <= 'Z') {
